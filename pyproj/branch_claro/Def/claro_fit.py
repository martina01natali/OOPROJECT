...
def fileinfo(TDIR, DIRPATH="*Station_1__*\Station_1__??_Summary\Chip_???\S_curve",
    ...
    print(f"Total number of bad files: {bad_counts}. Output paths to bad files are stored in {OUTBAD}")
    ...
    try:
        ...
        for i in all_params:
            if i == np.inf:
                ...
                raise ValueError(f'Fit returned a parameter that is < 1e-6 or negative. Please try with different GUESSES.')
